...
for (const member of graph.sources[0].records) {
  ...
  builder.materializeOne(member, "active_window", null);
  builder.materializeOne(member, "active_pane", null);
}
...
const observations = fixture.cases.map((entry) => {
  ...
  const matched = selection.count({ name, session_id: entry.session_id }) === 1;
