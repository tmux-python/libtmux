...
matching your own command line. Waiting on a channel sidesteps that entirely,
which is why the sections above do; scanning output is where you have to handle
it yourself.
...
Two commands run below. The first succeeds even though its own command line
contains `ERROR`, which is what defeats a pane-wide search; the second genuinely
fails.

The tag separates the command's output from the shell's echo, not one command's
output from the last one's. A capture still reads the whole pane, so a scan
after the second command would also see the first command's lines. Clear the
pane between commands when each verdict has to stand on its own.
...
>>> error_pane.send_keys(
...     "clear; printf 'out%sError: no disk\\n' :; "
...     f"tmux wait-for -S {ch}"
... )
>>> server.wait_for(ch)
>>> command_output(error_pane)
...
the result lines your command owns. The tag is assembled by the shell, as in the
section above, so `result:` never appears in what you typed and a wrapped echo
...
...     pane.send_keys(f'tmux wait-for -S multi-shell-{i}')
>>> for i in range(len(panes)):
...     server.wait_for(f'multi-shell-{i}')
...
>>> # Wait for all
>>> wait_all_complete(panes, 'COMPLETE', timeout=10.0)
True
...
>>> timeout_pane.send_keys('tmux wait-for -S timeout-shell-ready')
>>> server.wait_for('timeout-shell-ready')
...
>>> retry_pane.send_keys('tmux wait-for -S retry-shell-ready')
>>> server.wait_for('retry-shell-ready')
...
>>> queue_pane.send_keys('tmux wait-for -S queue-shell-ready')
>>> server.wait_for('queue-shell-ready')
...
>>> state_pane.send_keys('tmux wait-for -S state-shell-ready')
>>> server.wait_for('state-shell-ready')

>>> def run_state_machine(pane, states, timeout_per_state=10.0):
