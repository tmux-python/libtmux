...
class ServerConnection:
    ...
    def tmux_version(self) -> str | None:
        ...
        Probes ``tmux -V`` once. Answers ``None`` when the binary is missing or
        its version cannot be parsed, so a caller rendering version-gated argv
        can fall back to assuming the newest tmux rather than failing.
        ...
        The probe is memoized, so repeated reads cost one ``tmux -V``:

        >>> conn = ServerConnection()
        >>> conn.tmux_version() == conn.tmux_version()
        True
        """
