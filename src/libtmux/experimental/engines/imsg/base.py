...
from libtmux.experimental.engines.imsg.types import ImsgFrame, ImsgHeader
from libtmux.experimental.engines.imsg.v8 import ProtocolV8Codec
from libtmux.experimental.engines.registry import register_engine


def _select_codec(version: int | str) -> ImsgProtocolCodec:
    """Return the codec for a tmux imsg protocol version (only v8 is supported)."""
    if str(version) == ProtocolV8Codec.version:
        return ProtocolV8Codec()
    raise UnsupportedProtocolVersion(str(version))
...
class ImsgEngine:
    ...
    def run(self, request: CommandRequest) -> CommandResult:
        ...
        peer_id = int(self.protocol_version or ProtocolV8Codec.version)
        retries_remaining = 1

        while True:
            sock: socket.socket | None = None
            codec = _select_codec(peer_id)
            try:
