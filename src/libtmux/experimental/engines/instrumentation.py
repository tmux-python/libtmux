...
classes. :class:`~libtmux.experimental.engines.base.TmuxEngine` is a
``runtime_checkable`` protocol, so a decorator substitutes for the real engine
anywhere one is accepted, and costs nothing where it is absent.
...
class CountingSink:
    ...
    def before_command(self, request: CommandRequest) -> int:
        ...
        Counting happens here, on ``request.args``, because
        ``encode_direct_argv`` flattens :class:`CommandSeparator` into a plain
        string. An observer reading the encoded argv would report no inlining.
        """
        self.requests += 1
...
class InstrumentedEngine:
    ...
    >>> from libtmux.experimental.engines.base import CommandRequest
    >>> counts = CountingSink()
    >>> engine = InstrumentedEngine(MockEngine(), counts)
    >>> _ = engine.run_batch([CommandRequest.from_args("list-panes")] * 3)
    >>> counts.requests
    3
    """
